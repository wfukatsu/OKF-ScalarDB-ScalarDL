...
4. **`status: deprecated` の概念は設計判断の根拠にしない。** 既存システムの調査目的でのみ使う。
   `maintenance: archived` が付いたバージョンは特に注意（下の「アーカイブ済みバージョン」を参照）。
...
`maintenance: archived` / `archived: true` が付いたバージョンは、**上流がドキュメントサイトから
削除した**バージョンです。バンドルには削除直前のスナップショットが残っています。

- **`resource` のリンクは 404 します。** ドキュメントサイトに該当ページが存在しません。
  検証可能な出典として提示できるのは `sources[]`（生成元コミットへのパーマリンク）だけです。
- **再生成されません。** 上流に元データが無いため、内容は削除時点で固定です。
- **新規設計の根拠にしないこと。** そのリリースで動いている既存システムの調査にのみ使います。
- 引用するときは**アーカイブ済みである旨を必ず明示**すること。

対象は各製品の `index.md` の Versions 表（Maintenance 列が `archived`）と、
frontmatter の `archived_versions` で確認できます。
...
- **`resource` を持ちません。** 公開された正典 URL が存在しないためです。出典は
  `sources[]` に**タイトルのみ**で記録されています（`access: internal`）。
  引用するときは URL ではなく「Scalar 製品価格表 2024年7月1日版」のように出典名で示すこと。
...
`pricing/` 配下だけは例外で、`resource` がありません。出典が社内資料であり公開 URL を
持たないためです。`sources[]` のタイトルと `price_list_version` を出典として示してください。
