...
| `maintenance` | 意味 | 使いどころ |
|---|---|---|
| `supported` | 上流がメンテナンス中 | 新規設計・既存プロジェクトいずれも可 |
| `unmaintained` | 上流にドキュメントはあるがメンテナンス終了 | 既存システムの調査。新規設計には使わない |
| `archived` | **上流がドキュメントサイトから削除済み** | 既存システムの調査のみ。下記の注意を必ず読むこと |

`archived` は上流にもう元データが無い状態です。バンドルに残っているのは削除直前の
スナップショットで、再生成されず、`resource` のリンクは 404 します。検証可能な出典は
`sources[]` のコミットパーマリンクだけです。引用する際は必ずアーカイブ済みである旨を添えてください。
...
エディションごとに何が使えるか・いくらかは
[エディション別 機能・提供物マトリクス](../pricing/edition-feature-matrix.md) と
[価格・ライセンス](../pricing/index.md) にまとまっています（**Pay as you go / 月額 / 年額の定価のみ**）。
