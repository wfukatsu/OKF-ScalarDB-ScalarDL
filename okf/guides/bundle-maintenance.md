...
上流が古いバージョンをドキュメントサイトから削除しても、そのディレクトリはバンドルに残ります。
**残っているものは自動的に「アーカイブ済み」として扱われます。** 手作業は不要です。

生成時、`okf/products/<product>/` 配下にある `\d+\.\d+` 形式のディレクトリのうち、
上流で発見されなかったものを検出し、次の処理をします。

- 製品の `index.md` の Versions 表に `archived` として載せ、Docs 列を
  `— (removed upstream)` にする。あわせて「Archived versions」節で理由を説明する
- 製品 `index.md` の frontmatter に `archived_versions` を追加する
  （`supported_versions` には入れない）
- バージョンの `index.md` に `maintenance: archived` / `archived: true` /
  `archived_at` を付け、冒頭に注記を挿入する（`archived_at` は初回のみ記録。
  再実行しても書き換わりません）
- 概念数を数え直して `.okf-state.json` とルート索引の合計に含める
  （state に記録が残っていない場合は、`.md` の総数から `index.md` を引いて算出します）

つまり、**上流から消えたバージョンをバンドルから消したくなければ、何もしなくて構いません**。

本当に不要なら、ディレクトリを削除したうえで `make build` を実行してください。
`.okf-state.json` は再生成されるので手で編集する必要はありません。

### アーカイブ済みバージョンの注意

`resource` のリンクは 404 します。上流にページが存在しないためです。検証可能な出典として
残るのは `sources[]` のコミットパーマリンクだけです。この点は
`guides/how-ai-agents-use-this-bundle.md` にも AI 向けの鉄則として書いてあります。
