---
type: Reference
title: Scalar DB Supported Databases
description: Scalar DB supports the following databases and the databases that are compatible with those databases. Cassandra Cosmos DB DynamoDB MySQL PostgreSQL
resource: https://scalardb-community.scalar-labs.com/docs/3.4/scalardb-supported-databases/
tags:
- scalardb-community
- v3.4
- phase:implement
- section:reference
- unmaintained
status: deprecated
product: scalardb-community
product_title: ScalarDB Community
version: '3.4'
doc_id: scalardb-supported-databases
lifecycle_phase: implement
breadcrumb:
- Reference
generated:
  by: process:okf-build/1.0.0
  at: '2026-08-24T00:15:48Z'
sources:
- id: docs-scalardb-community
  resource: https://github.com/scalar-labs/docs-scalardb-community/blob/71d199cb0df1c638bd7e305b64fa09fc7236e5c4/versioned_docs/version-3.4/scalardb-supported-databases.mdx
  title: ScalarDB Community documentation source (MDX)
  author: process:scalar-labs/docs-scalardb-community
  last_modified: '2025-04-07T11:32:02Z'
---

# Scalar DB Supported Databases

Scalar DB supports the following databases and the databases that are compatible with those databases.
* Cassandra
* Cosmos DB
* DynamoDB
* MySQL
* PostgreSQL

## Compatibility matrix

| Scalar DB Version   | Cassandra 3.11 | Cassandra 3.0    | Cassandra 2.2   | Cosmos DB | DynamoDB  | MySQL 8.0  | MySQL 5.7  | PostgreSQL 13    | PostgreSQL 12   | PostgreSQL 11  | Oracle 19.0.0.0.ru-2021-04.rur-2021-04.r1 | Oracle 12.2.0.1.ru-2021-04.rur-2021-04.r1 | Oracle 12.1.0.2.v24   |
|:--------------------|:---------------|:-----------------|:----------------|:----------|:----------|:-----------|:-----------|:-----------------|:----------------|:---------------|-------------------------------------------|-------------------------------------------|-----------------------|
| Scalar DB 3.1.0 |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |
| Scalar DB 3.0.0 |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |
| Scalar DB 2.4.1 |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |
| Scalar DB 2.3.0 |  ✅  |  ✅  |  ✅  |  ✅  |  ✅  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |
| Scalar DB 2.2.2 |  ✅  |  ✅  |  ✅  |  ✅  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |
| Scalar DB 2.1.0 |  ✅  |  ✅  |  ✅  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |
| Scalar DB 2.0.1 |  ✅  |  ✅  |  ✅  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |  ❌  |
