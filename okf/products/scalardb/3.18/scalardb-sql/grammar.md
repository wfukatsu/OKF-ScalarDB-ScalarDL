...
resource: https://scalardb.scalar-labs.com/docs/3.18/scalardb-sql/grammar/
tags:
- scalardb
- v3.18
- phase:implement
...
sources:
- id: docs-scalardb
  resource: https://github.com/scalar-labs/docs-scalardb/blob/6126dfe2f56389351d88b134752618641f9771dd/versioned_docs/version-3.18/scalardb-sql/grammar.mdx
...
- DCL
  ...
  - [DROP ROLE](#drop-role)
  - [GRANT ... TO ROLE](#grant--to-role)
  - [REVOKE ... FROM ROLE](#revoke--from-role)
  - [GRANT ROLE](#grant-role)
  - [REVOKE ROLE](#revoke-role)
  - [REVOKE ADMIN OPTION](#revoke-admin-option)
- Others
  - [USE](#use)
  - [BEGIN](#begin)
  ...
  - [SHOW ROLE USERS](#show-role-users)
  - [SHOW ROLE GRANTS](#show-role-grants)
- Literal
  - [Text](#text)
  - [Numeric](#numeric)
...
The `ADD COLUMN` subcommand adds a new column to a table.
...
Examples of `ADD COLUMN` are as follows:
...
Examples of building statement objects for `ADD COLUMN` are as follows:
...
The `DROP COLUMN` subcommand drops a column from a table.
...
Examples of `DROP COLUMN` are as follows:
...
Examples of building statement objects for `DROP COLUMN` are as follows:
...
The `RENAME COLUMN` subcommand renames a column in a table.
...
An example of `RENAME COLUMN` is as follows:
...
An example of building statement objects for `RENAME COLUMN` is as follows:
...
The `RENAME TO` subcommand renames a table.
...
An example of `RENAME TO` is as follows:
...
An example of building statement objects for `RENAME TO` is as follows:
...
The `ALTER COLUMN SET DATA TYPE` subcommand changes the data type of a column in a table.
...
An example of `ALTER COLUMN SET DATA TYPE` is as follows:
...
An example of building statement objects for `ALTER COLUMN SET DATA TYPE` is as follows:
...
having_predicate: having_key operator <literal> | having_key BETWEEN <literal> AND <literal> | having_key [NOT] LIKE <pattern> [ESCAPE <escape character>] | having_key IS [NOT] NULL
having_key: identifier | aggregate_function | <alias>
...
- Columns in the `SELECT` clause must either appear in the `GROUP BY` clause or be used with an aggregate function.

`HAVING` clause:

- Filters groups based on aggregate function results.
- Can reference aggregate functions directly or by alias.
- Predicates must be an OR-wise of `having_and_predicates` (known as disjunctive normal form) or an AND-wise of `having_or_predicates` (known as conjunctive normal form).
- When connecting multiple `having_and_predicates` or `having_or_predicates`, which have more than one predicate, you need to put parentheses around `having_and_predicates` and `having_or_predicates`.
- Supports comparison operators (`=`, `<>`, `!=`, `>`, `>=`, `<`, `<=`), `BETWEEN`, `LIKE`, and `IS [NOT] NULL` predicates.

Aggregate functions:

- Supported functions: `COUNT`, `SUM`, `AVG`, `MIN`, `MAX`.
- `COUNT(*)` counts all rows; `COUNT(<column name>)` counts non-null values. The result type is `BIGINT`.
- `SUM` accepts numeric types (`INT`, `BIGINT`, `FLOAT`, `DOUBLE`). It returns `BIGINT` for `INT` and `BIGINT`, and `DOUBLE` for `FLOAT` and `DOUBLE`.
- `AVG` accepts numeric types (`INT`, `BIGINT`, `FLOAT`, `DOUBLE`). Returns `DOUBLE`.
- `MIN` accepts any comparable type. Returns the same type.
- `MAX` accepts any comparable type. Returns the same type.
- Only `COUNT` supports `*` as an argument.

`ORDER BY` clause:

- You can specify `order` for columns, aggregate functions, or aliases in the `ORDER BY` clause.
...
Examples of `SELECT` with aggregation are as follows:
...
-- With aggregate function alias and HAVING
SELECT age, COUNT(*) AS cnt FROM ns.user GROUP BY age HAVING cnt > 1;

-- With HAVING using aggregate function directly
SELECT age, COUNT(*) FROM ns.user GROUP BY age HAVING COUNT(*) > 1;

-- With aggregate function alias and multi-predicate HAVING
SELECT age, COUNT(*) AS cnt FROM ns.user GROUP BY age HAVING cnt > 1 AND cnt < 100;

-- With GROUP BY, WHERE, ORDER BY using alias, and LIMIT
SELECT age, SUM(height) AS total_height FROM ns.user WHERE name LIKE 'A%' GROUP BY age ORDER BY total_height DESC LIMIT 10;
```

Examples of building statement objects for `SELECT` with aggregation are as follows:

```java
// With aggregate function
SelectStatement statement1 =
    StatementBuilder.select(Projection.function(FunctionRef.count()))
        .from("ns", "user")
        .build();
...
SelectStatement statement2 =
    StatementBuilder.select(
            Projection.column("age"), Projection.function(FunctionRef.count()))
        .from("ns", "user")
...
SelectStatement statement3 =
    StatementBuilder.select(
            ...
            Projection.function(FunctionRef.count("height")),
            Projection.function(FunctionRef.sum("height")),
            Projection.function(FunctionRef.avg("height")),
            Projection.function(FunctionRef.min("height")),
            Projection.function(FunctionRef.max("height")))
        .from("ns", "user")
        .groupBy(ColumnRef.of("age"))
        .build();
...
SelectStatement statement4 =
    StatementBuilder.select(
            Projection.column("age"),
            Projection.function(FunctionRef.count()).as("cnt"))
        .from("ns", "user")
        .groupBy(ColumnRef.of("age"))
...
SelectStatement statement5 =
    StatementBuilder.select(
        ...
        .from("ns", "user")
        .groupBy(ColumnRef.of("age"))
        .having(HavingPredicate.function(FunctionRef.count()).isGreaterThan(Value.of(1L)))
        .build();
...
SelectStatement statement6 =
    StatementBuilder.select(
        ...
        .groupBy(ColumnRef.of("age"))
        .having(HavingPredicate.column("cnt").isGreaterThan(Value.of(1L)))
        .and(HavingPredicate.column("cnt").isLessThan(Value.of(100L)))
        .build();
...
SelectStatement statement7 =
    StatementBuilder.select(
            Projection.column("age"),
            Projection.function(FunctionRef.sum("height")).as("total_height"))
        .from("ns", "user")
        .where(Predicate.column("name").isLike(Value.of("A%")))
        .groupBy(ColumnRef.of("age"))
...
-- With operation attributes
INSERT INTO ns.tbl (c1, c2, c3, c4, c5) VALUES (10, 'aaa', 1.23, 100, true) WITH 'attribute1' = 'value1' AND 'attribute2' = 'value2';
```
...
// With operation attributes
InsertStatement statement5 = StatementBuilder.insertInto("ns", "tbl")
    .columns("c1", "c2", "c3", "c4", "c5")
    .values(Value.ofInt(10), Value.ofText("aaa"), Value.of(1.23F), Value.of(100L), Value.of(true))
    .withAttribute("attribute1", "value1")
    .withAttribute("attribute2", "value2")
...
The `CREATE USER` command creates a new user with the specified username, password, authentication methods, and user attributes.

#### Grammar

```sql
CREATE USER <username> [WITH] {PASSWORD <password> | AUTH_METHOD <authentication_method_list> | SUPERUSER | NO_SUPERUSER} ...
```

Where `<authentication_method_list>` is one or more of `USERPASS` and `OIDC`, separated by commas:

The `PASSWORD <password>` clause and the `AUTH_METHOD` clause are independent. The `PASSWORD <password>` clause sets the user's password credential, while the `AUTH_METHOD` clause controls which authentication methods are enabled for the user.

- If you don't specify a password for a user, the user is created without a password.
- If you specify the `AUTH_METHOD` clause, the user is created with the specified authentication methods.
- If you don't specify the `AUTH_METHOD` clause, the default authentication method (`USERPASS`) is used.
...
-- Create a user with the USERPASS authentication method.
CREATE USER user4 AUTH_METHOD USERPASS;

-- Create a user with both USERPASS and OIDC authentication methods.
CREATE USER user5 AUTH_METHOD USERPASS, OIDC;

-- Create a user with a password and the OIDC authentication method as a superuser.
CREATE USER user6 WITH PASSWORD 'password6' AUTH_METHOD OIDC SUPERUSER;
```
...
CreateUserStatement statement1 =
    StatementBuilder.createUser("user1").withPassword("password1").withSuperuser().build();

// Create a user with a password.
CreateUserStatement statement2 =
    StatementBuilder.createUser("user2").withPassword("password2").build();

// Create a user without a password.
CreateUserStatement statement3 = StatementBuilder.createUser("user3").build();

// Create a user with the OIDC authentication method.
CreateUserStatement statement4 =
    StatementBuilder.createUser("user1")
        .withAuthenticationMethods(Set.of(AuthenticationMethod.OIDC))
        .build();

// Create a user with both USERPASS and OIDC authentication methods.
CreateUserStatement statement5 =
    StatementBuilder.createUser("user1")
        .withAuthenticationMethods(
            Set.of(AuthenticationMethod.USERPASS, AuthenticationMethod.OIDC))
        .build();
```

### ALTER USER

The `ALTER USER` command changes the password, authentication methods, and user attributes of the specified user.

#### Grammar

```sql
ALTER USER <username> [WITH] {PASSWORD <password> | AUTH_METHOD <authentication_method_list> | SUPERUSER | NO_SUPERUSER} ...
```

Where `<authentication_method_list>` is `NONE` or one or more of `USERPASS` and `OIDC`, separated by commas:

- If you specify the `AUTH_METHOD` clause, the user's authentication methods are updated to the specified methods.
- If you specify `AUTH_METHOD NONE`, all authentication methods are cleared from the user.
- If you don't specify the `AUTH_METHOD` clause, the user's current authentication methods remain unchanged.
- If you specify the `SUPERUSER` attribute for a user, the user becomes a superuser.
- If you specify the `NO_SUPERUSER` attribute for a user, the user becomes a normal user.
...
-- Change a user's authentication method to OIDC.
ALTER USER user1 AUTH_METHOD OIDC;

-- Clear all authentication methods from a user.
ALTER USER user1 AUTH_METHOD NONE;
```
...
```java
// Change the password of a user.
AlterUserStatement statement1 =
    StatementBuilder.alterUser("user1").withPassword("password1").build();

// Change a user to a superuser.
AlterUserStatement statement2 = StatementBuilder.alterUser("user1").withSuperuser().build();

// Change a user's authentication method to OIDC.
AlterUserStatement statement3 =
        StatementBuilder.alterUser("user1")
            .withAuthenticationMethods(Set.of(AuthenticationMethod.OIDC))
            .build();

// Clear all authentication methods from a user.
AlterUserStatement statement4 =
        StatementBuilder.alterUser("user1")
            .withAuthenticationMethods(Set.of())
            .build();
```
...
- If you specify the `WITH GRANT OPTION` option or the `GRANT OPTION` privilege, the user can grant privileges to other users or roles.
...
- If the target user has the `UPDATE` or `DELETE` privilege, you cannot revoke the `SELECT` privilege from them.
...
The `CREATE ROLE` command creates a new role with the specified role name.
...
An example of `CREATE ROLE` is as follows:
...
An example of building statement objects for `CREATE ROLE` is as follows:

```java
// Create a role.
CreateRoleStatement statement = StatementBuilder.createRole("role1").build();
```
...
An example of `DROP ROLE` is as follows:
...
An example of building statement objects for `DROP ROLE` is as follows:

```java
// Delete a role.
DropRoleStatement statement = StatementBuilder.dropRole("role1").build();
```

### GRANT ... TO ROLE

The `GRANT ... TO ROLE` command grants privileges to the specified role.
...
- You must grant `INSERT` and `UPDATE` privileges together.
- To grant a role the `UPDATE` or `DELETE` privilege, the target role must have the `SELECT` privilege.
- If you specify the `WITH GRANT OPTION` option or the `GRANT OPTION` privilege, any user who has the role can grant privileges to other users or roles.

#### Examples

Examples of `GRANT ... TO ROLE` are as follows:

```sql
-- Grant the SELECT privilege on a table to a role.
GRANT SELECT ON ns.tbl TO ROLE role1;

-- Grant the SELECT privilege on tables to roles.
GRANT SELECT ON ns.tbl1, ns.tbl2 TO ROLE role1, role2;

-- Grant the SELECT privilege on all tables in a namespace to a role.
GRANT SELECT ON NAMESPACE ns TO ROLE role1;

-- Grant all privileges and GRANT OPTION on a table to a role.
GRANT ALL ON ns.tbl TO ROLE role1 WITH GRANT OPTION;
```

Examples of building statement objects for `GRANT ... TO ROLE` are as follows:

```java
// Grant the SELECT privilege on a table to a role.
GrantPrivilegeToRoleStatement statement1 =
    StatementBuilder.grantToRole(Privilege.SELECT).onTable("ns", "tbl").toRole("role1").build();

// Grant the SELECT privilege on tables to roles.
GrantPrivilegeToRoleStatement statement2 =
    StatementBuilder.grantToRole(Privilege.SELECT)
        .onTable("ns", "tbl1", "ns", "tbl2")
        .toRole("role1", "role2")
        .build();

// Grant the SELECT privilege on all tables in a namespace to a role.
GrantPrivilegeToRoleStatement statement3 =
    StatementBuilder.grantToRole(Privilege.SELECT).onNamespace("ns").toRole("role1").build();

// Grant all privileges and GRANT OPTION on a table to a role.
GrantPrivilegeToRoleStatement statement4 =
    StatementBuilder.grantToRole(Privilege.values())
        .onTable("ns", "tbl")
        .toRole("role1")
        .withGrantOption()
        .build();
...
The `REVOKE ... FROM ROLE` command revokes privileges from the specified role.
...
- You must revoke `INSERT` and `UPDATE` privileges together.
- If the target role has the `UPDATE` or `DELETE` privilege, you cannot revoke the `SELECT` privilege from it.

#### Examples

Examples of `REVOKE ... FROM ROLE` are as follows:

```sql
-- Revoke the SELECT privilege on a table from a role.
REVOKE SELECT ON ns.tbl FROM ROLE role1;

-- Revoke the SELECT privilege on tables from roles.
REVOKE SELECT ON ns.tbl1, ns.tbl2 FROM ROLE role1, role2;

-- Revoke the SELECT privilege on all tables in a namespace from a role.
REVOKE SELECT ON NAMESPACE ns FROM ROLE role1;

-- Revoke all privileges and GRANT OPTION on a table from a role.
REVOKE ALL, GRANT OPTION ON ns.tbl FROM ROLE role1;
```

Examples of building statement objects for `REVOKE ... FROM ROLE` are as follows:

```java
// Revoke the SELECT privilege on a table from a role.
RevokePrivilegeFromRoleStatement statement1 =
    StatementBuilder.revokeFromRole(Privilege.SELECT).onTable("ns", "tbl").fromRole("role1").build();

// Revoke the SELECT privilege on tables from roles.
RevokePrivilegeFromRoleStatement statement2 =
    StatementBuilder.revokeFromRole(Privilege.SELECT)
        .onTable("ns", "tbl1", "ns", "tbl2")
        .fromRole("role1", "role2")
        .build();

// Revoke the SELECT privilege on all tables in a namespace from a role.
RevokePrivilegeFromRoleStatement statement3 =
    StatementBuilder.revokeFromRole(Privilege.SELECT).onNamespace("ns").fromRole("role1").build();

// Revoke all privileges and GRANT OPTION on a table from a role.
RevokePrivilegeFromRoleStatement statement4 =
    StatementBuilder.revokeFromRole(Privilege.values())
        .onTable("ns", "tbl")
        .fromRole("role1")
        .build();
```
...
The `GRANT ROLE` command grants roles to the specified users or roles.

#### Grammar

```sql
GRANT ROLE <role name> [, <role name>] ... TO [USER] <username> [, <username>] ... [WITH ADMIN OPTION]
GRANT ROLE <role name> [, <role name>] ... TO ROLE <role name> [, <role name>] ... [WITH ADMIN OPTION]
```

- If you specify the `WITH ADMIN OPTION` option, the grantee can grant the role to other users or roles.

#### Examples

Examples of `GRANT ROLE` are as follows:
...
GRANT ROLE role1, role2 TO user1, user2;

-- Grant a role to a user with ADMIN OPTION.
GRANT ROLE role1 TO user1 WITH ADMIN OPTION;

-- Grant a role to another role.
GRANT ROLE role1 TO ROLE role2;

-- Grant a role to another role with ADMIN OPTION.
GRANT ROLE role1 TO ROLE role2 WITH ADMIN OPTION;
```

Examples of building statement objects for `GRANT ROLE` are as follows:

```java
// Grant a role to a user.
GrantRoleToUserStatement statement1 =
    StatementBuilder.grantRole("role1").toUser("user1").build();

// Grant roles to users.
GrantRoleToUserStatement statement2 =
    StatementBuilder.grantRole("role1", "role2").toUser("user1", "user2").build();

// Grant a role to a user with ADMIN OPTION.
GrantRoleToUserStatement statement3 =
    StatementBuilder.grantRole("role1").toUser("user1").withAdminOption().build();

// Grant a role to another role.
GrantRoleToRoleStatement statement4 =
    StatementBuilder.grantRole("role1").toRole("role2").build();

// Grant a role to another role with ADMIN OPTION.
GrantRoleToRoleStatement statement5 =
    StatementBuilder.grantRole("role1").toRole("role2").withAdminOption().build();
```

### REVOKE ROLE

The `REVOKE ROLE` command revokes roles from the specified users or roles.

#### Grammar

```sql
REVOKE ROLE <role name> [, <role name>] ... FROM [USER] <username> [, <username>] ...
REVOKE ROLE <role name> [, <role name>] ... FROM ROLE <role name> [, <role name>] ...
```

#### Examples

Examples of `REVOKE ROLE` are as follows:
...
REVOKE ROLE role1 FROM ROLE role2;
```

Examples of building statement objects for `REVOKE ROLE` are as follows:

```java
// Revoke a role from a user.
RevokeRoleFromUserStatement statement1 =
    StatementBuilder.revokeRole("role1").fromUser("user1").build();

// Revoke roles from users.
RevokeRoleFromUserStatement statement2 =
    StatementBuilder.revokeRole("role1", "role2").fromUser("user1", "user2").build();

// Revoke a role from another role.
RevokeRoleFromRoleStatement statement3 =
    StatementBuilder.revokeRole("role1").fromRole("role2").build();
```

### REVOKE ADMIN OPTION

The `REVOKE ADMIN OPTION` command revokes the admin option for roles from the specified users or roles. The grantee keeps the roles but can no longer grant them to others.

#### Grammar

```sql
REVOKE ADMIN OPTION FOR ROLE <role name> [, <role name>] ... FROM [USER] <username> [, <username>] ...
REVOKE ADMIN OPTION FOR ROLE <role name> [, <role name>] ... FROM ROLE <role name> [, <role name>] ...
```

#### Examples

Examples of `REVOKE ADMIN OPTION` are as follows:

```sql
-- Revoke the admin option for a role from a user.
REVOKE ADMIN OPTION FOR ROLE role1 FROM user1;

-- Revoke the admin option for a role from another role.
REVOKE ADMIN OPTION FOR ROLE role1 FROM ROLE role2;
```

Examples of building statement objects for `REVOKE ADMIN OPTION` are as follows:

```java
// Revoke the admin option for a role from a user.
RevokeAdminOptionForRoleFromUserStatement statement1 =
    StatementBuilder.revokeAdminOptionForRole("role1").fromUser("user1").build();

// Revoke the admin option for a role from another role.
RevokeAdminOptionForRoleFromRoleStatement statement2 =
    StatementBuilder.revokeAdminOptionForRole("role1").fromRole("role2").build();
```
...
Examples of `BEGIN` are as follows:
...
-- With operation attributes
BEGIN WITH 'cc-transaction-isolation' = 'SNAPSHOT';
```

Examples of building statement objects for `BEGIN` are as follows:
...
// With operation attributes
BeginStatement statement4 =
    StatementBuilder.begin()
        .withAttribute("cc-transaction-isolation", "SNAPSHOT")
        .build();
```
...
Examples of `START TRANSACTION` are as follows:
...
-- With operation attributes
START TRANSACTION WITH 'cc-transaction-isolation' = 'SNAPSHOT';
```

Examples of building statement objects for `START TRANSACTION` are as follows:
...
// With operation attributes
StartTransactionStatement statement4 =
    StatementBuilder.startTransaction()
        .withAttribute("cc-transaction-isolation", "SNAPSHOT")
        .build();
```
...
- `authenticationMethods`: `TEXT` - the configured authentication methods (for example, `USERPASS`, `USERPASS, OIDC`)
...
- `grantedToUser`: `BOOLEAN` - whether the privilege is directly granted to the user
- `rolesProvidingPrivilege`: `TEXT` - a comma-separated list of roles that provide this privilege to the user
...
The `SHOW ROLES` command shows all roles or the roles granted to the specified user.

When showing all roles (`SHOW ROLES`), this command returns the following columns:

- `roleName`: `TEXT` - a role name
- `grantedRoles`: `TEXT` - the roles granted to this role (comma-separated, with `(admin option)` suffix if the grant includes admin option)

When showing all roles available to the user directly or through role hierarchy (`SHOW ROLES FOR`), this command returns the following columns:

- `roleName`: `TEXT` - a role name
- `adminOptionOnUser`: `BOOLEAN` - whether this role was directly granted to the user with admin option
- `grantedRoles`: `TEXT` - the roles granted to this role (comma-separated, with `(admin option)` suffix if the grant includes admin option)
...
-- Show roles granted to the specified user
SHOW ROLES FOR [USER] <username>
```

#### Examples

Examples of `SHOW ROLES` are as follows:

```sql
-- Show all roles
SHOW ROLES;

-- Show roles granted to user1
SHOW ROLES FOR user1;
```

Examples of building statement objects for `SHOW ROLES` are as follows:

```java
// Show all roles
ShowRolesStatement statement1 = StatementBuilder.showRoles().build();

// Show roles granted to user1
ShowRolesStatement statement2 = StatementBuilder.showRoles().forUser("user1").build();
```

### SHOW ROLE USERS

The `SHOW ROLE USERS` command shows the users the specified role is directly granted to.

This command returns the following columns:

- `username`: `TEXT` - a username
- `adminOption`: `BOOLEAN` - whether the user has admin option for the role

#### Grammar

```sql
SHOW ROLE USERS FOR <role name>
```

#### Examples

An example of `SHOW ROLE USERS` is as follows:

```sql
-- Show users who have been granted role1
SHOW ROLE USERS FOR role1;
```

An example of building statement objects for `SHOW ROLE USERS` is as follows:

```java
// Show users who have been granted role1
ShowRoleUsersStatement statement = StatementBuilder.showRoleUsers("role1").build();
```

### SHOW ROLE GRANTS

The `SHOW ROLE GRANTS` command shows the privileges directly granted to the specified role.
...
```sql
SHOW ROLE GRANTS FOR <role name>
```

#### Examples

An example of `SHOW ROLE GRANTS` is as follows:

```sql
-- Show privileges granted to role1
SHOW ROLE GRANTS FOR role1;
```

An example of building statement objects for `SHOW ROLE GRANTS` is as follows:

```java
// Show privileges granted to role1
ShowRoleGrantsStatement statement = StatementBuilder.showRoleGrants("role1").build();
```
